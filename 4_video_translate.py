...
class TranscribeWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress_signal.emit(55, "Transcribing speech via Google...")
            recognizer = sr.Recognizer()
            with sr.AudioFile(tmp_wav) as source:
                audio = recognizer.record(source)
            os.remove(tmp_wav)

            self.progress_signal.emit(90, "Processing transcript...")
            text = recognizer.recognize_google(audio)
            self.progress_signal.emit(100, "Done.")
            self.done_signal.emit(text)

        except sr.UnknownValueError:
            self.error_signal.emit("Could not understand the audio. Try a clearer video.")
        except sr.RequestError as e:
            self.error_signal.emit(f"Google STT error: {e}")
...
class ExportWorker(QThread):
    ...
    def __init__(self, gif_paths, output_path, fps=12):
        super().__init__()
...
class WordCard(QFrame):
    def __init__(self, word, gif_path=None):
        ...
        if has_gif:
            ...
            self.movie = None
            gif_lbl.setStyleSheet("color: #555; font-size: 20px; border: none;")
            gif_lbl.setText("—")
        ...
        status_lbl = QLabel("matched" if has_gif else "no sign")
        status_lbl.setAlignment(Qt.AlignCenter)
        status_lbl.setFont(QFont("Segoe UI", 8))
        status_lbl.setStyleSheet(f"color: {ACCENT if has_gif else '#666'}; border: none;")
...
class VideoTranslateApp(QMainWindow):
    ...
    def on_transcript_ready(self, text):
        ...
        self.gif_paths_for_export = [p for _, p in matched if p]
        for word, gif_path in matched:
            self.cards_flow.addWidget(WordCard(word, gif_path))
        self.translate_btn.setEnabled(True)
        if self.gif_paths_for_export:
            self.export_btn.setEnabled(True)
        self.status_lbl.setText(
            f"{len(self.gif_paths_for_export)} of {len(matched)} words matched to signs."
        )
    ...
    def export_video(self):
        ...
        self.export_btn.setEnabled(False)
        self.translate_btn.setEnabled(False)
        self.export_worker = ExportWorker(self.gif_paths_for_export, out)
